...
@
\end{center}
\caption{PCA score plot. The displayed components can be specified with \texttt{parCompVi} (plotting \textbf{par}ameter specifying the \textbf{comp}onents: \textbf{V}ector of \textbf{i}nteger type)}
\label{fig:pca}
\end{figure}
...
\item As for PCA, the default \texttt{predI} value is NA: components are computed until either R2Y of the component is < 0.01 or Q2Y is < 0 (for more than 100 observations) or 0.05 otherwise.
\end{enumerate}
...
sacPlsLs <- roplsF(dataMatrix, matrix(sampleMetadata[, "genderVc"], ncol = 1),
permI = 100, parDevNewL = FALSE, plotVc = "permutation")
@
\end{center}
...
sacOplLs <- roplsF(dataMatrix, matrix(sampleMetadata[, "genderVc"], ncol = 1),
predI = 1, orthoI = NA, parDevNewL = FALSE, plotVc = "x-score")
@
\end{center}
