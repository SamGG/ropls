...
For OPLS(-DA) models, the output from \emph{opls} method therefore includes:
\begin{itemize}
...
\end{itemize}
As for the classical $VIP$, we still have the mean of $VIP_{4,pred}^2$ (and of $VIP_{4,ortho}^2$) which equals 1.
