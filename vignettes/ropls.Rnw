...
For OPLS(-DA) models, the output from the \emph{opls} method therefore includes:
\begin{itemize}
\item the \texttt{vipVn} vector which corresponds to the $VIP_{4,pred}$ metric measuring the variable importance in prediction,
\item the \texttt{orthoVipVn} vector which is the $VIP_{4,ortho}$ metric measuring the variable importance in orthogonal modeling.
\end{itemize}
