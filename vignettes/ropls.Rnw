...
A summary of the model (8 components were selected) is printed. In addition the default \texttt{summary} 4-plot figure displays:
\begin{enumerate}
\item An inertia \texttt{overview} barplot (or \emph{scree} plot): The graphic here suggests that 3 components may be sufficient to capture most of the inertia (see below),
\item \texttt{outlier} diagnostics (distances within and orthogonal to the projection plane; \cite{Hubert2005}): The name of the samples with a high value for at least one of the distances are indicated,
\item The \texttt{x-score} plot,
\item The \texttt{x-loading} plot.
\end{enumerate}

To plot the scores only (Figure \ref{fig:pca}):
...
\end{center}
\caption{PCA \texttt{score} plot. The displayed components can be specified with \texttt{parCompVi} (plotting \textbf{par}ameter specifying the \textbf{Comp}onents: \textbf{V}ector of 2 \textbf{i}ntegers)}
\label{fig:pca}
\end{figure}
...
@
\end{center}
\caption{PCA \texttt{score} plot colored according to \emph{gender}.}
\label{fig:pcaMah}
\end{figure}
...
@
\end{center}
\caption{PLS-DA model of the \emph{gender} response (\texttt{score} plot). The number of components and the cumulative R2X, R2Y and Q2Y are indicated below the plot.}
\label{fig:pls}
\end{figure}

Note:
\begin{enumerate}
\item For (O)PLS modeling of a single response, the significance of R2Y and Q2Y are estimated permutation testing (and displayed as the first \texttt{permutation} graphic) in order to check that the R2Y and Q2Y of the model with the true response are higher than the values obtained after randomly permutating the values of the response (The default number of permutations is 100 and can be increased with the \texttt{permI} parameter to further confirm the significance)
...
We see that our model with 3 predictive (\texttt{pre}) components has significant and quite high (cum)ulative R2Y and Q2Y.
...
@
\end{center}
\caption{OPLS-DA model of the \emph{gender} response (\texttt{score} plot). The predictive component is displayed as abscissa and the orthogonal component as ordinate.}
\label{fig:opl}
\end{figure}
