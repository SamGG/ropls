...
A summary of the model (8 components were selected) is printed. In addition a 4-plot figure displays an inertia barplot, outlier diagnostics, and the score and loading plots. To plot the scores only (Figure \ref{fig:pca}):

<<eval=FALSE,results=hide>>=
plot(sacurine.pca, plotVc = "x-score")
@
...
plot(sacurine.pca, plotVc = "x-score", parDevNewL = FALSE)
@

\end{center}
\caption{PCA score plot. The displayed components can be specified with \texttt{parCompVi} (plotting \textbf{par}ameter specifying the \textbf{Comp}onents: \textbf{V}ector of 2 \textbf{i}ntegers)}
\label{fig:pca}
\end{figure}
...
<<eval=FALSE,results=hide>>=
genderFc <- sampleMetadata[, "genderFc"]
plot(sacurine.pca, plotVc = "x-score",
...
Note that the plotting \textbf{par}ameter to be used \textbf{As} \textbf{Col}ors (\textbf{F}actor of \textbf{c}haracter type or \textbf{V}ector of \textbf{n}umeric type) has a length equal to the number of rows of the dataMatrix matrix (ie of samples) and that this qualitative or quantitative variable is converted into colors (by using an internal palette or color scale, respectively). We could have visualized the age of the individuals by specifying \texttt{parAsColFcVn = sampleMetadata[, "ageVn"]}.


\subsection{Partial least-squares: PLS and PLS-DA}

For PLS (and OPLS), the Y response(s) must be provided. Y can be either a numeric vector (respectively matrix) for single (respectively multiple) (O)PLS regression, or a character factor for (O)PLS-DA classification as in the following example (The score plot is displayed on Figure \ref{fig:pls}).
...
plot(sacurine.plsda, plotVc = "x-score", parDevNewL = FALSE)
@
\end{center}
...
We see that 3 predictive ('pre') components were computed and that the (cum)ulative R2Y and Q2Y are quite high. Let us assess the significance of these values by comparing to models generated after random permutation of the labels (Figure \ref{fig:plsPer}).

<<eval=FALSE,results=hide>>=
sacurine.plsPer <- opls(dataMatrix, genderFc,
permI = 100)
@
<<echo=FALSE>>=
sacurine.plsPer <- opls(dataMatrix, genderFc,
permI = 100, printL = FALSE, plotL = FALSE)
@

\begin{figure}
\begin{center}
<<label=figPlsPer,fig=TRUE,echo=FALSE,results=hide>>=
plot(sacurine.plsPer, plotVc = "permutation", parDevNewL = FALSE)
@
\end{center}
...
The R2Y and Q2Y values of the model trained on the models with randomly permuted response labels are all below the values of the model with the true response (horizontal lines). Significance of our PLS-DA model should be further confirmed by using a higher number of permutations.


\subsection{Orthogonal partial least squares: OPLS and OPLS-DA}

To perform OPLS(-DA), we set \texttt{orthoI} (number of components which are \textbf{ortho}gonal; \textbf{I}nteger) to either a specific number of orthogonal components, or to NA. Let us build an OPLS-DA model of the 'gender' response (Figure \ref{fig:plsPer}). Note that for OPLS modeling of a single response, the number of predictive component is 1.
...
plot(sacurine.oplsda, plotVc = "x-score", parDevNewL = FALSE)
@
\end{center}
...
@
<<echo=FALSE>>=
sacurine.oplsda <- opls(dataMatrix, genderFc, predI = 1, orthoI = NA, subset = "odd", plotL = FALSE)
@
